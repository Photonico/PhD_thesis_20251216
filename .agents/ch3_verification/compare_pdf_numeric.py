...
with contextlib.redirect_stdout(io.StringIO()):
    import recompute_optics as raw

FIGURES=[('3.1_dielectric.pdf','dielectric'),('4.1_absorption.pdf','absorption_nm^-1'),
         ('4.2_refractive.pdf','n'),('4.3_extinction.pdf','k'),('4.4_reflectivity.pdf','R'),('4.5_energy-loss.pdf','L')]
...
for filename,quantity in FIGURES:
    ...
    fig={'file':str(THESIS/'figures_ch3'/filename),'axes':[{'panel':i,'clip_rect_pt':list(r),**cal[k]} for i,(k,r) in enumerate(frames)],'curves':[]}
    current=None
    for drawing in drawings:
        ...
        energy,values=raw.cache[(phase,channel,'archived')];yraw=values[field]
...
out=ROOT/'pdf-vs-h5-numeric-audit.json';out.write_text(json.dumps(report,indent=2)+'\n')
print('REPORT',out)
