...
Before comparing states across the Brillouin zone, we recall why its boundary points can represent the same crystal momentum.
...
The Brillouin zone contains one set of inequivalent crystal momenta, with opposite boundaries identified by these reciprocal-lattice translations.
...
We will use this identification when closing a path across the zone boundary and when identifying the momenta fixed by time reversal.
...
coincident partners of the two selected pairs are not drawn separately.
...
Orange circles identify the contacts in (c) and (d).
...
and (d) crossing within the selected subspace.}
...
In (d), both crossing pairs belong to the selected subspace on either side of the crossing.
The two pairs exchange their order in energy, but both remain included in the projector.
...
whereas changing the hamiltonian or the loop can change that result continuously.
...
so it gives an equivalent momentum.
...
Equations~\eqref{parity_product_at_trim_topology} and~\eqref{fu_kane_index_topology}
then have the same counting as equations~\eqref{eq:proj3_parity_product} and~\eqref{eq:proj3_z2_index} in chapter~\ref{cha:project_beryllene}.
